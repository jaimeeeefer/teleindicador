...
export async function buscarTeleindicador() {
    const input = document.getElementById("numeroEst");
    ...
    const tipoTren = document.getElementById("tipoTrenCustom").textContent;
    ...
    try {
        ...
        if (trenes.length < 25){
            resultadoEst.innerHTML = `Estas son todas las ${tipoPanelSeleccionado} disponibles para la estación <strong>${codigo.trim()+" - "+estaciones[codigo.replace(/^0+/, '')] || codigo}</strong>.`;
            finCargarMas();
        } else {
            resultadoEst.innerHTML = `Se ha(n) encontrado <strong>${trenes.length}</strong> tren(es) en <strong>${codigo.trim()+" - "+estaciones[codigo.replace(/^0+/, '')] || codigo}</strong>.`;
        }
        renderizarPanelTeleindicador(trenes);
    } catch (e) {
        console.error("Error en la búsqueda del teleindicador:", e);
        resultadoEst.textContent = "Error: " + e.message;
        tbody.innerHTML = `<tr><td colspan="5" style="text-align:center">Error al consultar.</td></tr>`;
    }
}

export async function buscarTeleindicadorSilent() {
    const input = document.getElementById("numeroEst");
    let codigo = input.dataset.codigo || "";
    const estaciones = getEstaciones();
    ...
    // Estado de carga
    const resultadoEst = document.getElementById("resultadoEst");
    resultadoEst.append(" - Actualizando...");
